...
var asana_key = '1NK7gy6i.fsjuHQMpnXQCVKhNbWBC3jg'; //process.env.ASANA_KEY
...
function createIssue(issue){  
  ...
  var auth = 'Basic ' + new Buffer(asana_key+":").toString('base64')
    ...
    , assignee_id = userMapping[issue.issue.user.login]
    , due_date = issue.issue.milestone.due_on
    , title = issue.issue.title
    , workspaceId = workspaces['datakind.org']
    , data;

  data = {data: { name: title, workspace: workspaceId, assignee: assignee_id, notes: description, due_on: due_date }}//{data: {name: title, workspace: workspaceId, assignee: assignee_id}}
  
  console.log({data: { name: title, workspace: workspaceId, assignee: assignee_id, notes: description, due_on: due_date }}) 
