...
function createTaskFromIssue(issue){  
  ...
  request.post({
    ...
    json: data,
    headers: {"Authorization":auth}
  }, proxy(githubToAsanaCallback, self)); 
}
...
function addComment(ghRes){
  var auth = "Basic " + new Buffer(asana_key + ":").toString('base64')
    , taskId = parseTaskId(ghRes.issue.body)
    , text = ghRes.comment.body
    , url = asana_base_url + "/tasks/" + taskId + "/stories"
    , data = {data:{"text": text}};

  console.log([text, url, auth, data]);
...
function completeTaskFromIssue(issue){
  var auth = 'Basic ' + new Buffer(asana_key+":").toString('base64')
    , taskId = parseTaskId(issue.issue.body)
    , assigneeId = userMapping[issue.issue.assignee.login]
    , workspaceId = workspaces['datakind.org']
    , data;

console.log('issue id is', taskId)
  ...
  request.put({
    url: asana_base_url + "/tasks/" + taskId,
    json: data,
    headers: {"Authorization": auth},
  }, apiCallback)
}	
...
exports.issueEvent = function(req,res){
  ...
  if (action === 'closed') completeTaskFromIssue(body)
  res.send('All quiet on the server front')
};

function parseTaskId(description){
  var taskId = description.match(/Asana\sTask\s#[0-9]*/g) 
  taskId = taskId[0].split(' ').reverse()[0].slice(1)
  return taskId
}
